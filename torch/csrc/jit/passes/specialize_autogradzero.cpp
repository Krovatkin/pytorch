...
void specializeAutogradZero(Graph& g, bool in_profiling_mode) {
  ...
  for (auto it = g.nodes().begin(); it != g.nodes().end(); ++it) {
    ...
    switch (n->kind()) {
      ...
      case prim::profile: {
        if (n->inputs().size() > 0)
        {
          std::cout << "propagating state " << (int)state[n->input()] << " from " <<  n->input()->debugName() << " to " << n->output()->debugName() << std::endl;
          state[n->output()] = state[n->input()];
        }
        break;
      }
      case prim::BailOut: {
        if (auto ptt = n->output()->type()->expect<ProfiledTensorType>())
        {
  ...
  std::cout << "after specializeToZero\n";
  g.dump();
}
